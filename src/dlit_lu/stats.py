...
def plot_distribution(
    df: pd.DataFrame, columns: list, save_path: pathlib.Path, category: str = "Res"
):
    """
    ...
    for col in columns:
        ...
        plt.xticks(
            np.linspace(x_min, x_max, num=15)
...
def plot_boxplots(
    ...
    for col in columns:
        ...
        plt.axhline(
            median, color="purple", linestyle="-", label=f"Median (50%): {median:.4f}"
        )
        plt.axhline(
            percentile_10,
            color="blue",
            linestyle="--",
            label=f"10%: {percentile_10:.4f}",
        )
        plt.axhline(
            percentile_90,
            color="brown",
            linestyle="--",
            label=f"90%: {percentile_90:.4f}",
        )
...
def plot_zscore_distributions(
    ...
    fig, axes = plt.subplots(
        num_cols, 3, figsize=(15, 5 * num_cols), sharey=True
    )  # Ensure same y-scale
    ...
    for i, col in enumerate(columns):
        for j, suffix in enumerate(["zscore", "robust_zscore", "modified_zscore"]):
            ...
            ax.axvline(
                x=0, color="red", linestyle="--", label="Mean/Median"
            )  # Mean/Median line
