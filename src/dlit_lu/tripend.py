...
class TEConstraintProcessor(constraint.ConstraintProcessor):
    ...
    def merge_zone_data(
        ...
        zonal_data = zonal_data[zone_col_in_by + [base_year_column] + future_year_columns]
        return zonal_data
    ...
    def aggregate_to_sector_p_m(
        ...
        sector_agg = merged_df.groupby([sector_id,"p","m"], as_index=False)[val_cols].sum()
...
class ByTeTransformation:
    def __init__(self, df: pd.DataFrame, base_year_column: str):
        ...
        self.df = self.df[self.df["period"].isin([1, 2, 3, 4])]  # Keep only period categories 1, 2, 3, and 4  
...
def run(config: inputs.DLitConfig):
    ...
    LOG.info("Instantiating GrowthCalculator and ConstraintCalculation from constraint module")
    ...
    ntem_zone_te = {
        "ntem_hb_prod": pd.read_csv(config.tripend.ntem_zone_hb_prod)[[zone_id, "p", "m", base_year_column] + future_year_columns],
        "ntem_hb_attr": pd.read_csv(config.tripend.ntem_zone_hb_attr)[[zone_id, "p", "m", base_year_column] + future_year_columns],
        "ntem_nhb_prod": pd.read_csv(config.tripend.ntem_zone_nhb_prod)[[zone_id, "p", "m", base_year_column] + future_year_columns],
        "ntem_nhb_attr": pd.read_csv(config.tripend.ntem_zone_nhb_attr)[[zone_id, "p", "m", base_year_column] + future_year_columns],
    }
                            ...
                            if key in results:
                                ...
                                if df is not None and f"{sector}nm" in df.columns:
                                    ...
                                    if sector_list:
                                        ...
                                        df_filtered = df[df[f"{sector}nm"].isin(sector_list)]
                                    else:
                                        # Use the original DataFrame if sector_list is not provided
                                        df_filtered = df
                                    
                                    if not df_filtered.empty:  # Ensure it's not empty after filtering
    ...
    for category in categories:
        for data_type in te:
            ...
            zone_etmt_growth = zone_estimated_growth[zone_index_columns + future_year_columns]
            ...
            agg_zone_scaled_etmt_growth = zone_scaled_etmt_growth.groupby(sector_index_columns)[
                future_year_columns
            ].sum()
            ...
            agg_zone_bg_growth = zone_bg_growth.groupby(sector_index_columns)[future_year_columns].sum()
            ...
            if sector_list:
                ...
                for key, value in data_files_and_names.items():
                    filtered_data = value["data"][value["data"][f"{sector}nm"].isin(sector_list)]
